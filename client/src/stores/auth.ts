...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function verifyOtp(phone: string, code: string, mode?: string) {
    const api = useApi();
    ...
    if (data.ok) {
      ...
      if (data.data.isNew && user.value?.role === 'patient') {
        router.push('/profile');
  ...
  return { token, user, isAuthenticated, isProfileComplete, isDoctor, isPatient, login, verifyOtp, fetchProfile, logout };
});
